...
面向多模态数据的混合检索增强生成（RAG）系统，支持文本搜图和图搜文本。集成 CLIP 跨模态嵌入、4种向量索引算法（HNSW / FAISS IVF-PQ / LSH / Annoy）、混合检索策略（稠密向量 + 稀疏 BM25 + 动态权重分配），结合大模型 API 实现端到端的检索与智能生成。
...
- **多模态检索**：文本搜图 + 图搜文本，CLIP 统一嵌入空间（512维）
- **4种向量索引**：HNSW (hnswlib C++)、FAISS IVF-PQ、余弦 LSH (SimHash)、Annoy，完整对比实验
- **混合检索**：稠密向量 + 稀疏 BM25 + 查询路由器动态权重分配
- **中文友好**：中文查询自动翻译为英文检索，LLM 中文回答
- **LLM API**：支持 DeepSeek、通义千问、OpenAI、SiliconFlow 等兼容接口，通过 `.env` 配置
- **索引持久化**：首次构建后自动保存到磁盘，重启秒级加载
- **Gradio Web 界面**：检索结果画廊 + LLM 回答
...
基于 Flickr30k 真实数据集（29K 图片, 145K 标注），完成 6 组实验：

| 实验 | 研究问题 | 核心发现 |
|------|---------|---------|
| **E1: ANN 对比** | 4种向量索引算法在跨模态数据上的表现 | HNSW (hnswlib) 综合最优：R@10=54.8%, 查询延迟 <1ms |
| **E2: 降维影响** | PCA/UMAP 降维对检索质量的影响 | 跨模态检索对降维较敏感，PCA 256d 下 R@10 下降明显 |
| **E3: 混合检索** | 稠密 + 稀疏融合 vs 纯向量检索 | 自适应混合 R@1=42.5%，纯向量 R@1=24.0%，绝对提升 18.5 个百分点 |
| **E4: 跨模态对称** | 文搜图 vs 图搜文双向对比 | I→T R@10=72.3% > T→I R@10=55.0%，存在不对称性 |
| **E5: RAG 质量** | LLM 生成的回答质量评估 | 人工评分（相关性 / 准确性 / 有用性） |
| **E6: 规模扩展** | 1K→29K 规模下延迟和召回变化 | HNSW 延迟亚线性增长，全量仍 <1ms |

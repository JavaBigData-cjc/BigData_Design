...
|------|-----------|------------|------------|----------|-----------|-----------|
| 1K | **1.000** | 0.375 | **1.000** | 0.27ms | 0.77ms | 0.46ms |
| 5K | **0.759** | 0.207 | 0.483 | 0.64ms | 1.74ms | 0.56ms |
| 10K | **0.723** | 0.138 | 0.508 | 1.21ms | 1.71ms | 0.64ms |
| 20K | **0.607** | 0.107 | 0.329 | 1.22ms | 1.19ms | 0.70ms |
| 29K | **0.550** | 0.090 | 0.265 | 1.34ms | 1.13ms | 0.77ms |

**关键发现**：
1. **HNSW 在所有规模上全面领先**：29K 时 R@10=55.0%，是 FAISS (9.0%) 的 6.1x，Annoy (26.5%) 的 2.1x
2. **HNSW 延迟增长亚线性**：1K→29K 数据量 29x 增长，延迟仅从 0.27ms → 1.34ms（5x）
3. **FAISS IVF-PQ 扩展性差**：R@10 从 1K 的 37.5% 剧烈下降至 29K 的 9.0% — PQ 量化对跨模态检索的破坏随规模放大
...
| LLM 生成 | ⏳ 待做 | 通义千问/DeepSeek API RAG 答案生成 (E5) |
...
3. **混合检索（自适应向量+BM25）是最重要的创新**：R@1 从 24.0% 提升至 42.5%（+77.1%）
